...
const inlineImages = () => {
  const raw = (process.env.CK_CONFIG_INLINE_IMAGES || '').toString().trim().toLowerCase();
...
const main = async () => {
  ...
  const banner =
    ...
    `// Contains the analytics page export embedded from ${path.relative(root, configPath)}${inlineImages() ? ' with inlined images' : ' without inlined images'}.\n`;
