...
- **Datasource bootstrap waits for shared inventory work**: Guided line-group step targets can opt into `dataSourceBootstrap.waitForGuidedReservationSync: true` when their datasource rows read shared inventory that may still be updating from another step. They can also set `dataSourceBootstrap.waitForSharedDataMutations: true` so the step waits for in-flight follow-up actions that mutate the datasource's shared form keys before it starts `fetchDataSource`, preventing Leftover Bank style screens from bootstrapping or warning against stale inventory.
