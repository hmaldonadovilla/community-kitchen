...
- **Home leave guard for incomplete dedup keys**: Configure `actionBars.system.home.dedupIncompleteDialog` to show a confirmation dialog when users try to leave the form while reject-dedup fields are incomplete. On confirm, the app navigates Home and can delete the current persisted record first (`deleteRecordOnConfirm`, default `true`).
