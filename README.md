...
- **File uploads**: Set `Type` to `FILE_UPLOAD` and provide `uploadConfig` in the Config column (JSON). Supported keys:
  ...
  - `linkCapture` (optional QR/link capture; set `{ "enabled": true, "mode": "driveQr", "allowManualPaste": true, "dedupeBy": "driveFileId" }` to let users scan or paste existing Google Drive file links into the same field without uploading a new file)
    ...
    - Set `instruction` to the localized sentence shown above the camera and `sessionTtlMinutes` to a value from 1 to 30. `hideCloseOnIos: true` hides the page-owned Close action on iOS. Add `commitOnReturnOnIos: true` to hide Finish there and treat returning with the native browser X as completion of the accepted batch. Android and desktop retain their page-owned actions.
    - The continuous scanner opens its Firebase-hosted page and requests camera access immediately while Apps Script prepares the authoritative session in parallel. Scans made during preparation are queued. The camera remains active for multiple codes, and the scanner shows the server result for every candidate.
    - **Finish and add receipts** revalidates accepted files and appends them once through an idempotent, field-scoped Apps Script commit on Android and desktop. With `commitOnReturnOnIos`, users scan multiple receipts and use the native browser X when finished; the foreground form then commits the authoritative batch without relying on a suspended background-tab message. Returning with no accepted receipt cancels the empty session.
