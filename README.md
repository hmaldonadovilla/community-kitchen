...
- **Reservation release hooks**: Source forms can now configure `reservationLifecycle.releaseOnDelete` so active reservations are released automatically when the source record is deleted (including dedup-key delete/recreate flows). The same source form can add a `lifecycle.rules[]` entry of type `releaseStaleReservations` so the daily `2am` lifecycle trigger releases stale unfinished reservations without custom code.
