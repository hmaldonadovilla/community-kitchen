...
- **Form-level conditional field disable rules**: Configure `fieldDisableRules` at the form/dashboard level to make the edit view read-only when a `when` condition matches, with optional `bypassFields` to keep specific fields editable (for example, lock all fields when `DATE` is in the future except `COOK`).
  - Guided-step locks can target a single step by adding `__ckStep` in the rule condition (for example, lock only `orderInfo` when a dedicated order-lock flag is set).
  - `when` clauses can also reference request params via `__ckRequestParam_<name>` (for example `__ckRequestParam_admin`).
  - For admin-only unlock flows, add a Summary `updateRecord` button that clears the configured lock flag and gate its `visibility.showWhen` with `__ckRequestParam_admin`.
  - For the dedicated `ready-for-production-order-lock` rule, you can use `?unlock=<record_id>` in the web-app URL to bypass the lock for that specific record. Set `unlockSet` on that rule to automatically clear the lock flag or update scalar fields when unlock is used.
...
- **Conditional templates (reports + emails)**: `templateId` / `pdfTemplateId` / `emailTemplateId` / `summaryHtmlTemplateId` can be a string, a language map, or a `cases` selector that picks a template based on record field values (first match wins; supports per-language template IDs per case). `when` clauses can compare DATE fields to fixed cutoff dates, so historical records can keep a legacy summary/report while newer records use the current version.
- **Custom BUTTON fields**: Add BUTTON questions for Doc template PDF previews (`action: "renderDocTemplate"`; opens a new tab and navigates it directly to the generated PDF blob), Markdown template previews (`action: "renderMarkdownTemplate"`), HTML template previews (`action: "renderHtmlTemplate"`), open a saved URL from a field (`action: "openUrlField"`), create a new record with preset values (`action: "createRecordPreset"`), or mutate the current record (`action: "updateRecord"`). For PDF buttons you can optionally set `button.loadingLabel` to customize the "Generating..." copy. Markdown/HTML render buttons cache successful browser render results; set `button.cacheScope: "template"` only for static templates so repeat opens reuse one cache entry across records/draft changes. `openUrlField` buttons render as real new-tab links when possible, and saved `pdfUrl` reports open through the in-app PDF preview on iOS standalone/PWA installs to avoid replacing the CK page. `openUrlField` buttons can also set `disableWhenValueMissing: true` to stay visible but disabled until the target URL exists. `updateRecord` buttons can declare a reusable `dependencyGuard` that previews impacted downstream records, shows a configurable dialog only when matches exist, and either applies configured downstream mutations before saving the current record or blocks the source update with `mode: "block"`. They can set `progressDialog` for blocking save copy and `suppressStatusFeedback: true` when that dialog should replace the form-level Saving/Saved banner. Blocking dialogs can include a generated `{{recordsList}}` from `dialog.recordList.template` using `{{target.FIELD_ID}}` placeholders. Buttons can be placed inline (`form`), in the edit Summary menu (`formSummaryMenu`), in the Summary bottom bar (`summaryBar`), in the top action bar under the header (all views: `topBar` or per view: `topBarList` / `topBarForm` / `topBarSummary`), in the List bottom bar (`listBar`), or as a template-only action (`htmlTemplate`) invoked from bundled HTML via `data-ck-action`. HTML templates can come from a Drive file id or a bundled key like `bundle:checklist_am.summary.html` (embedded from `/docs/templates` at build time and rendered client-side; may call `fetchDataSource` when projection placeholders like `{{FIELD.PROJ}}` are present). Security: Drive-sourced HTML templates must not include `<script>` tags; dynamic behavior should be implemented in bundled templates. HTML templates can pass runtime top-level value patches to `updateRecord` buttons with `data-ck-action-value-field` plus `data-ck-action-value-source`; templates that validate an input first can trigger a hidden `data-ck-action` button after validation succeeds. Set a configured `confirm.title` value to `""` when the confirmation dialog should have no title. Templates can include a clickable icon placeholder `{{FILES_ICON(FIELD_ID)}}` to open the Photos overlay in read-only mode.
- **Template caching (HTML/Markdown)**: HTML/Markdown templates are cached in Apps Script `CacheService` for faster backend reads, and successful browser render results are cached under the app/server cache version. Browser cache keys are record-scoped by default; static Markdown/HTML button templates can set `button.cacheScope: "template"` to avoid repeat server calls when the same top-bar/help content is opened across records or draft changes. Static Markdown buttons are also warmed after page load when possible, so opening help/instructions templates does not wait on the first render request. You can force-refresh backend template content via Create/Update All Forms (template cache epoch), and optionally configure per-form template cache TTL (seconds) via the dashboard JSON `templateCacheTtlSeconds` (max 6h due to Apps Script limits).
...
- Prefer: install triggers via **Community Kitchen → Install Triggers (Options + Response indexing + Daily analytics + Daily lifecycle + Scheduled alerts)** so manual edits automatically bump `Data Version` + etags, daily analytics reconciliation runs, config-driven lifecycle status rules are evaluated at 2am, and configured operational alert checks run automatically.
...
- **File uploads**: Set `Type` to `FILE_UPLOAD` and provide `uploadConfig` in the Config column (JSON). Supported keys:
  ...
  - `waitMessages.title` / `waitMessages.saveTitle` / `waitMessages.removeSelectedTitle` / `waitMessages.scanTitle` / `waitMessages.save` / `waitMessages.removeSelected` / `waitMessages.scan` (optional localized title and copy for blocking upload, remove, and in-flight QR scan transactions; set a title value to `""` to hide the title line; omitted messages fall back to generic file-based system strings)
  - `linkLabel` (optional localized label template used for file links in Summary/PDF; e.g. `"Photo {n}"`)
  - `linkCapture` (optional QR/link capture; set `{ "enabled": true, "mode": "driveQr", "allowManualPaste": true, "dedupeBy": "driveFileId" }` to let users scan or paste existing Google Drive file links into the same field without uploading a new file)
    - Set `linkCapture.allowedMimeTypes` when captured Drive links need a type policy independent from uploaded files. It replaces the upload MIME/extension rules for link capture only; use `["*/*"]` to accept any non-folder file that passes Drive scope validation.
    - Add `validation: { "requireServerValidation": true, "includeUploadDestinationDrive": true, "includeUploadDestinationFolder": true, "rejectTrashed": true }` when captured links must belong to the same customer Drive/folder as `destinationFolderId`. Apps Script resolves the field configuration authoritatively and verifies Drive metadata before storing a scanner result. Use `validation.messages` to customize failures such as `notDriveFile`, `notAccessible`, `trashed`, and `outOfScope`; omitted messages fall back to `files.linkCapture.validation.*` system strings.
    - Set `instruction` to the localized sentence shown above the camera and `sessionTtlMinutes` to a value from 1 to 30. The scanner page always relies on the native browser X on iOS and shows one local **Close** action on Android/desktop. The legacy `commitOnReturnOnIos` key is still parsed for configuration compatibility but has no persistence role in the incremental flow.
    - The continuous scanner opens its Firebase-hosted page and requests camera access immediately. Apps Script preparation starts lazily with the first detected QR, so opening or waiting creates no session. Detected values are collected into micro-batches of up to three while the camera remains active. Within one scanner session, URL variants that resolve to the same canonical Drive file are coalesced, so repeated stickers for one delivery receipt do not trigger repeated Drive validation or record writes. On phone-width portrait screens the displayed camera region is reduced to leave more room for the scanned-item list; decoding still uses the stream's intrinsic resolution. The camera requests high-detail rear video and continuous focus when supported. The decoder checks centered source-resolution regions first, so users should hold one small QR code steady in the centre instead of moving the phone very close to the label.
    - Each micro-batch is an idempotent, field-scoped Apps Script transaction: unique files are authorised, their canonical links are appended in one atomic record mutation, and one complete authoritative field state is returned. **Added** means the receipt is already durable. The original one-candidate RPC remains as a compatibility fallback for an older deployment. iOS native X and Android/desktop Close never call commit or cancel; the originating file overlay uses the standard non-dismissible blocking overlay, with `waitMessages.scan` copy, only while relevant scan transactions are in flight.
    - The scanner runtime uses Firebase Hosting and Apps Script only. It does not require Cloud Run or a billing-enabled Google Cloud project.
  ...
  - Upload changes are saved through the queued record mutation lane. With `blockUntilSaved: true`, the file management overlay saves each photo add/remove action immediately and blocks until the record contains the final Drive URLs. Without blocking, the overlay stages photo and manually pasted link changes locally and sends one transaction when the user clicks **Save photos**. Continuous scanner results are applied from each scan's authoritative server response. If an upload transaction fails, the field keeps the local file changes visible and shows a retry action that sends the same transaction again.
...
- **Datasource cache policy (optional)**: Stable lookup sources such as customers, ingredients, recipes, and other master-data tables can set `dataSource.cachePolicy: "versioned"` so the browser reuses the local datasource cache across refreshes until the app/server cache version changes. For slow-changing master data that is already prefetched, `dataSource.forceRefreshMaxCacheAgeMs` lets generic force-refresh paths reuse a fresh cached response instead of issuing another server call. Add `dataSource.forceRefreshCacheScope: "dataSource"` only when sibling projections for the same datasource id can safely satisfy those ordinary refresh paths. Transactional sources can leave the default short TTL or set `cachePolicy: "none"` when persistence is not appropriate.
...
- **Trigger-based analytics module**: Configure `analytics.widgets` to compute `aggregate`/`arithmetic`/`script` metrics server-side, persist snapshots in hidden per-form analytics sheets, and render them in List (`placements: ["listView"]`) and/or the analytics page (`placements: ["analyticsPage"]`). Recomputes run on save/edit/follow-up actions plus a daily reconciliation trigger (`runDailyAnalyticsRecompute`). You can also configure fire-and-forget `analytics.pipelines` to surface date-driven export actions on the centralized analytics page; queued runs generate an `.xlsx` attachment and email it to the configured recipients. Pipeline types include `ingredientUsageReport`, `recordTableReport`, and `generatedBankReport` for multi-tab exports of records generated into a shared bank/repository. Ingredient usage exports support configured `Tbsp` to `gr` conversion, `gr` to `kg` conversion above 1000, and `EEE,dd-mmm-yyyy` report date placeholders.
...
- **Scheduled record alerts**: Configure `scheduledAlerts` on a form to run automatic time-based checks and email matching records for today's date. Alerts are generic and can filter by status and any field value, for example Meal Production Lunch at 13:00 and all services at 17:00 when records remain in `In progress` or `In production`.
...
- **Configurable utilisation conflict dialog**: Datasource-backed utilisation rows can define `utilisation.conflictDialog` so concurrency conflicts explain what happened. Immediate row syncs can offer two actions, such as using the authoritative available quantity or cancelling the attempted change. Guided step-commit conflicts return the user to the configured step after acknowledgement and deselect the rejected rows so dependent quantities and availability recalculate. Message templates support `{itemLabel}`, `{itemId}`, `{available}`, `{unit}`, `{availableWithUnit}`, `{requested}`, and `{current}`.
- **Source-first utilised-row visibility**: Utilisation-backed source-first selectors can include terminal bank statuses such as `used` in their datasource read filter, then rely on `sourceRows.includeWhen` to show rows only when there is free availability or the current record already uses that row. This lets a record release or adjust a fully consumed bank item without exposing unavailable items to other records.
- **Central landing page + admin canonicalization**: Opening the web app without `form` now renders a form-catalog landing page. Admin aliases (`?admin-true` or `?admin=1|true|yes|on`) are accepted and propagated as canonical `admin=true` in navigation links. Landing-page branding, copy, app grouping, per-card `imagePath` art, and header `logoFormKey` live in `docs/config/exports/<env>/landing_page.json`, which is embedded into the bundle at build time. App-launch links add a runtime `ckNav=open-app` marker so the destination shell suppresses the redundant boot wait copy after the landing page has already shown its navigation dialog.
