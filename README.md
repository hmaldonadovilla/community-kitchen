...
- **Unified Shell & Navigation**: Excel-style header (logo circle + form title) with a left drawer (Refresh / Language (optional) / Build), a global **top action bar** under the header (optional; driven by `BUTTON` placements), and a fixed bottom action bar that adapts per view (List/Summary/Form).
