...
- **Create/Copy UX (per form)**: Customize Create/Copy labels via `"createButtonLabel"` / `"copyCurrentRecordLabel"`, control copied data via `"copyCurrentRecordDropFields"` (blacklist) or `"copyCurrentRecordProfile"` (whitelist), and optionally show an informational dialog after copy via `"copyCurrentRecordDialog"`.
