...
- **Custom BUTTON fields**: Add `BUTTON` questions for **Doc template PDF previews** (`action: "renderDocTemplate"`), **Markdown template previews** (`action: "renderMarkdownTemplate"`), or **create a new record with preset values** (`action: "createRecordPreset"`). Buttons can be placed inline (`form`), in the edit Summary menu (`formSummaryMenu`), in the Summary bottom bar (`summaryBar`), in the top action bar under the header (all views: `topBar` or per view: `topBarList` / `topBarForm` / `topBarSummary`), or in the List bottom bar (`listBar`).
