...
- **Compact sentence rows for line items**: For guided/mobile-heavy line-item flows, set `lineItemConfig.ui.compactRows: true` and drive the first line, inline sentence controls, and row actions via `compactHeadlineRows`, `compactSentenceRows`, and `compactActions`. Headline parts can read either local hydrated fields or a selected datasource row with `sourceFieldId` + `sourcePath` (and `suffixSourcePath`) so compact rows stay config-driven instead of depending on hidden intermediary fields. Compact actions can also set `overlayLabel` separately from the button label when the overlay should stay titleless or use a different title.
...
- **Submit-time background follow-up (optional)**: Configure `submissionAfterSubmit` when some follow-up actions must complete before navigation (for example `CLOSE_RECORD`) while slower actions continue in the background after the user has already been redirected. The reusable config supports `preActions`, `backgroundActions`, `navigateTo`, and a configurable `feedbackDialog`.
