...
- **File uploads**: Set `Type` to `FILE_UPLOAD` and provide `uploadConfig` in the Config column (JSON). Supported keys:
  ...
  - `linkCapture` (optional QR/link capture; set `{ "enabled": true, "mode": "driveQr", "allowManualPaste": true, "dedupeBy": "driveFileId" }` to let users scan or paste existing Google Drive file links into the same field without uploading a new file)
    ...
    - The continuous scanner opens its Firebase-hosted page and requests camera access immediately. Apps Script preparation starts lazily with the first detected QR, so opening or waiting creates no session. Detected values are queued serially and the camera remains active for multiple codes.
    - Each scan is an idempotent, field-scoped Apps Script transaction: the file is authorised, its canonical link is appended, and the complete authoritative field state is returned. **Added** means the receipt is already durable. iOS native X and Android/desktop Close never call commit or cancel; the originating file overlay blocks only while relevant scan transactions are in flight.
