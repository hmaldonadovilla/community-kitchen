...
- **Lifecycle status automation**: Configure `lifecycle.rules` on a form to run generic date-driven status transitions from the daily Apps Script trigger (`runDailyLifecycleRecompute`, installed at `2am`). The first use is `Leftover Inventory`, which transitions `available -> expired` when `LEFTOVER_EXP_DATE < today`, but the rule shape is generic and can be reused for other stock/status tables.
