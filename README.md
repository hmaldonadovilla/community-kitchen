...
- **Background record freshness (optional, enabled by default)**: Add `"recordFreshness": { "enabled": true, "quietWindowMs": 30000 }` to the dashboard JSON column when you want an explicit per-form override for stale-record polling. While an existing record stays open, the app waits until the current record has had no successful record-related server activity for `quietWindowMs`, then sends a lightweight `getRecordVersion` ping. If the server version changed and there is no local record mutation in flight, the app reloads the latest snapshot automatically and shows a sync notice asking the user to review the step again.
