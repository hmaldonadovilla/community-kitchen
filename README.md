...
- **Computed Fields**: `derivedValue` can auto-calculate fields (including numeric formulas with `SUM(GROUP.FIELD)` aggregates), prefill DATE fields with today, map time-of-day into a default value, copy a default value from another field (e.g., NUMBER defaults), or compose readonly text from other fields with `op: "template"`, and keep them hidden/system-managed.
...
- **Field-payload line-item hydration**: `selectionEffects.type: "addLineItemsFromFieldPayload"` can hydrate a target line-item group from a serialized payload already present on the current row, which is useful when a selector should read from a shared inventory record and then materialize normalized internal rows.
...
- **Step-commit reservation mode**: Guided leftover-selection steps can opt into `reservation.commitMode: "step"` on a datasource row. In that mode, edits stay local while the user works through the step, and the app sends a single batched reservation plan when the user taps `Next`, replacing any stale reservations in the managed step scopes instead of issuing per-row writes while typing.
- **Datasource bootstrap wait for guided reservation syncs**: Guided line-group step targets can opt into `dataSourceBootstrap.waitForGuidedReservationSync: true` when their datasource rows read shared inventory that may still be updating from another step. The step waits for the current record's in-flight guided reservation draft sync to finish before it starts `fetchDataSource`, which prevents Leftover Bank style screens from bootstrapping against stale inventory immediately after managed rows were deleted elsewhere in the flow.
...
- **Draft autosave (optional)**: Enable background saves while editing by adding `"autoSave": { "enabled": true, "debounceMs": 2000, "status": "In progress" }` to the dashboard JSON column. Draft saves run without validation, bump `Updated At`, and write the configured `Status` (falls back to `statusTransitions.inProgress`). Records matching `statusTransitions.onClose` are read-only and do not autosave. The first time a user enters Create/Edit/Copy, the web app shows a one-time autosave explainer overlay (customize copy via `autosaveNotice.*` in `src/web/systemStrings.json`).
  ...
  - Optional dedup progress popup: set `autoSave.dedupCheckDialog` (checking/available/duplicate copy + auto-close durations) for a form-level non-dismissible dedup progress modal.
...
- **Auto-increment IDs**: Any `TEXT` field can be tagged with `"autoIncrement": { "prefix": "MP-AA", "padLength": 6 }` in its Config JSON. When the user leaves that field blank, Apps Script generates sequential IDs (e.g., `MP-AA000001`) and stores the counter in script properties so numbers stay unique across sessions. Set `padLength: 0` for variable-width ids like `LE-1`, `LE-2`, `LE-10`. For shared tables you can also partition the prefix by another field value with `prefixByValue`, for example `entireDish -> LE-` and `partialDish -> LP-`.
