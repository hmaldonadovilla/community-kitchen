...
- **File uploads**: Set `Type` to `FILE_UPLOAD` and provide `uploadConfig` in the Config column (JSON). Supported keys:
  ...
  - `waitMessages.save` / `waitMessages.removeSelected` (optional localized copy for blocking upload and remove transactions when `blockUntilSaved` is true; falls back to generic file-based system strings)
...
- **Step-commit reservation mode**: Guided leftover-selection steps can opt into `reservation.commitMode: "step"` on a datasource row. In that mode, the UI updates availability locally first, then live-syncs a batched step reservation plan on valid selection edits and quantity blur, replacing stale reservations in the managed step scopes instead of issuing per-row ledger writes while typing.
