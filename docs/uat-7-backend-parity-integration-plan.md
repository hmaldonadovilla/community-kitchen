...
One-hundred-fourteenth-pass results:

- `FormView.tsx`: extracted automatic single-choice default application into `useSingleChoiceDefaults`, moving top-level and line-item default selection, error cleanup, and selection-effect forwarding into a focused form hook. Current size: 11,054 lines.
- Validation for the pass used direct ESLint on touched files, `npx tsc --noEmit --pretty false`, focused line-item/form tests, `npm run lint:changed`, `git diff --check`, and `npm run build`.
- `App.tsx` was intentionally left untouched because the performance fix workstream is ongoing there.

One-hundred-fifteenth-pass results:

- `FormView.tsx`: extracted ordered-entry validation control into `useOrderedEntryValidationController`, moving guided validation-definition selection, first-blocked-field detection, ordered-entry guidance error construction, validation navigation triggering, and top-level gate ref wiring into a focused form hook. Current size: 10,775 lines.
...
One-hundred-twenty-fourth-pass results:

- `App.tsx`: extracted resolved top/bottom action bar composition into `AppActionBar` and form/list/summary rendering into `AppMainViews`, keeping the app shell focused on state and use-case wiring while moving view composition into app components. Current size: 14,248 lines.
