...
- Cloud Run `fetchAnalyticsDashboard` and bootstrap analytics payloads are implemented for persisted Sheets analytics snapshots. The deploy bundles `analytics_page.json` so dashboard sections and analytics-page pipeline metadata can load from Cloud Run. Cloud Run has non-production support for `queueAnalyticsPipelineRun` and `runQueuedAnalyticsPipelineJobs`, but production report requests are deliberately routed through Apps Script because the production Cloud Run environment is not enabled.
- Cloud Run scheduled job endpoints are available for `runQueuedAnalyticsPipelineJobs`, `runDailyAnalyticsRecompute`, and `runDailyLifecycleRecompute` under `POST /api/jobs/<jobName>` with `CK_SCHEDULER_SECRET` authentication. `DEPLOY_ENV=staging npm run deploy:cloud-scheduler` creates or updates Cloud Scheduler HTTP jobs for staging Cloud Run validation. Production analytics report requests use the Apps Script queue and Apps Script trigger instead.
- `saveSubmissionWithId`, `triggerFollowupAction(s)`, `fetchAnalyticsDashboard`, bundled HTML/Google Doc PDF render/preview functions, and template render functions can be included in `CK_HTTP_FUNCTIONS` for staging once the Cloud Run service account has editor/contributor access to the affected response sheets, template files/folders, PDF output folders, analytics export folders, and data-source spreadsheets. Do not include `queueAnalyticsPipelineRun`; report requests must stay on Apps Script. Include `uploadFiles` only after upload destinations are moved to Shared Drive folders shared with the runtime service account, or keep relying on the client fallback to Apps Script for service-account Drive quota errors.
