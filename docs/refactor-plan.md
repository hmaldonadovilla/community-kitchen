...
- Revise `WebFormTemplate` to a minimal HTML shell (meta + root div + preload JSON + script tag for the bundled React IIFE). Styling moves to CSS/inline assets compiled into the bundle.
...
- Dual-boot toggle: property flag (e.g., `CK_USE_REACT=true`) or query param to select React vs legacy template during rollout.
- Dogfood on non-critical forms; compare behavior and submissions for parity.
- Remove legacy bundle once parity is confirmed and flag is defaulted to React.
