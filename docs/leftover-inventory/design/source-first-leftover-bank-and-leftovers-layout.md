...
- nested inventory rendering: `dataSourceRows` under each meal row
- local row state per copied inventory row:
  - `LEFTOVER_SELECTED`
  - `LEFTOVER_USE_QTY`
  - `LEFTOVER_USAGE_MODE`
...
This model works for reservation-aware selection, but it duplicates the same leftover record across several meal rows and makes the UI less intuitive when the same leftover is split across dishes.

Two additional gaps are now also in scope:

- after `6. Leftovers`, newly created inventory rows are saved on the server but the local datasource cache is not refreshed immediately, so the user cannot reuse those leftovers without a hard refresh
- leftover inventory does not yet carry dietary applicability metadata, so `2. Leftover bank` cannot restrict leftovers to compatible dishes
...
This is not specific to leftovers. It is a reusable "inventory allocation matrix" pattern:
...
Source rows come from the inventory datasource:
...
Each leftover inventory record should expose the dietary applicability it supports.
...
- the derived dietary values must be saved on the inventory record

For `partialDish` leftovers:

- dietary applicability must also be captured and saved on the inventory record
- it should be derived using the same generic ingredient classification rules wherever possible
- if the ingredient-based derivation is not sufficient for a given partial leftover, the capture flow must still require or allow explicit dietary applicability input so the saved inventory record is complete

Recommended inventory field:
...
- newly created inventory records must be pushed into the client datasource state immediately
...
- after successful leftover creation, invalidate `Leftover Inventory Data`
...
For the `Leftover bank` step, the summary should come from the inventory row:
  ...
  "dataSourceAllocations": [
    {
      ...
      "postMutation": {
        "invalidateDataSources": ["Leftover Inventory Data"],
        "refreshDataSources": ["Leftover Inventory Data"]
      }
    }
  ]
}
...
2. Reuse the existing reservation service.

- keep the current ledger and inventory reconciliation model
...
- invalidate and refresh `Leftover Inventory Data`
- update local persisted cache without requiring hard refresh

6. Add dietary applicability capture, derivation, and filtering.

- derive dietary applicability from cooked dish ingredients when creating `LE` inventory
- capture or derive dietary applicability when creating `LP` inventory
