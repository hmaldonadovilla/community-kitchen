...
describe('selectionEffects setValue', () => {
  ...
  it('preserves refreshed source metadata when rebuilding matching auto rows', () => {
    (peekCachedDataSourcesById as unknown as jest.Mock).mockReturnValue([
      {
        items: [
          {
            id: 'recipe-1',
            NAME: 'Soup',
            INGREDIENTS: [{ ING: 'Carrot', UNIT: 'kg', QTY: 2 }]
          }
        ]
      }
    ]);
          ...
          selectionEffects: [
            {
              ...
              dataField: 'INGREDIENTS',
              dataSource: { id: 'Recipes Data', projection: ['id', 'NAME', 'INGREDIENTS'] },
              lineItemMapping: { ING: 'ING', UNIT: 'UNIT', QTY: 'QTY' },
              aggregateBy: ['ING', 'UNIT'],
              aggregateNumericFields: ['QTY'],
              preserveManualRows: false
            }
    ...
    let values: Record<string, any> = { RECIPE: 'recipe-1' };
    let lineItems: Record<string, any> = {
      LINES: [
        {
          id: 'line_1',
          autoGenerated: true,
          effectContextId: 'syncRecipeIngredientsFromSource',
          values: {
            ING: 'Carrot',
            UNIT: 'kg',
            QTY: 1,
    ...
    expect(lineItems.LINES).toHaveLength(1);
    expect(lineItems.LINES[0].id).toBe('line_1');
    expect(lineItems.LINES[0].values).toEqual(
      expect.objectContaining({
        ING: 'Carrot',
        UNIT: 'kg',
        QTY: '2',
  ...
  it('does not mutate datasource-backed auto rows when regeneration is identical', async () => {
    (fetchDataSource as unknown as jest.Mock).mockResolvedValue({
      items: [
        {
          id: 'recipe-1',
    ...
    const parentRowId = 'meal_1';
    const childKey = buildSubgroupKey('MEALS', parentRowId, 'INGREDIENTS');
    const effectContextId = `MEALS::${parentRowId}::syncRecipeIngredientsFromSource`;
    let values: Record<string, any> = {};
    let lineItems: Record<string, any> = {
      MEALS: [{ id: parentRowId, values: { RECIPE: 'Soup', RECIPE_SOURCE_ID: 'recipe-1' } }],
      [childKey]: [
        {
          id: 'ing_1',
    ...
    let valueMutationCount = 0;
    let lineItemStateChangeCount = 0;
    let lineItemMutationCallbackCount = 0;
    const setValues = (next: any) => {
      const previous = values;
      const resolved = typeof next === 'function' ? next(previous) : next;
      if (resolved !== previous) {
        valueMutationCount += 1;
        values = resolved;
      }
    };
    const setLineItems = (next: any) => {
    ...
    runSelectionEffects({
      ...
      setLineItems,
      onLineItemsMutated: () => {
        lineItemMutationCallbackCount += 1;
      },
      opts: {
        contextId: `MEALS::${parentRowId}::RECIPE`,
        lineItem: {
          groupId: 'MEALS',
          rowId: parentRowId,
          rowValues: { RECIPE: 'Soup', RECIPE_SOURCE_ID: 'recipe-1' }
        },
        forceContextReset: true,
        preferLookupSourceValue: true
      }
    ...
    expect(fetchDataSource).toHaveBeenCalledTimes(1);
    expect(valueMutationCount).toBe(0);
    expect(lineItemStateChangeCount).toBe(0);
    expect(lineItemMutationCallbackCount).toBe(0);
    expect(lineItems[childKey]).toHaveLength(1);
    expect(lineItems[childKey][0].values).toEqual(
      expect.objectContaining({
        ING: 'Carrot',
