...
describe('dataSources cache mutation', () => {
  const config: any = {
    ...
    mode: 'options',
    projection: ['id', 'LEFTOVER_ID', 'LEFTOVER_QTY']
  };
            ...
            return {
              withFailureHandler() {
                return {
                  fetchDataSource() {
                    success({
                      items: [{ id: 'leftover-1', LEFTOVER_ID: 'LE-1', LEFTOVER_QTY: 0 }],
                      totalCount: 1
                    });
                  }
                };
  ...
  test('mutates cached datasource items and persists the update', async () => {
    ...
    expect((updated as any)?.items?.[0]?.LEFTOVER_QTY).toBe(3);
    expect((peekCachedDataSource(config, 'EN') as any)?.items?.[0]?.LEFTOVER_QTY).toBe(3);
  });
  ...
  test('adopts persisted datasource updates from another tab', async () => {
    ...
    expect(initial?.items?.[0]?.LEFTOVER_QTY).toBe(0);

    const storageKey = Array.from({ length: windowStub.localStorage.length }, (_, index) => windowStub.localStorage.key(index))
      .filter((key): key is string => Boolean(key))
      .find(key => key.startsWith('ck.ds.Leftover%20Bank%20Data.EN.'));

    expect(storageKey).toBeTruthy();
    const nextValue = JSON.stringify({
      savedAtMs: Date.now(),
      response: {
        items: [{ id: 'leftover-1', LEFTOVER_ID: 'LE-1', LEFTOVER_QTY: 4 }],
        totalCount: 1
      }
    });
    ...
    expect((peekCachedDataSource(config, 'EN') as any)?.items?.[0]?.LEFTOVER_QTY).toBe(4);
  });
});
