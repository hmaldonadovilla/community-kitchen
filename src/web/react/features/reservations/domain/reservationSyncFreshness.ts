export type GuidedReservationSyncFreshness = {
  recordId: string;
  stepId?: string | null;
...
export type GuidedReservationSyncWaitResult = {
  ok: boolean;
  message?: string;
...
export const shouldUseReservationSyncFreshnessForDataSource = (args: {
  freshness?: GuidedReservationSyncFreshness | null;
...
export const shouldRefreshDataSourcesAfterReservationPlan = (args: {
  availabilityCount?: number | null;
  updatedRows?: number | null;
  reservationsReleased?: number | null;
}): boolean => {
  ...
  return Number(args.reservationsReleased || 0) > 0;
};
