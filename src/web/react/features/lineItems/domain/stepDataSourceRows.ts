...
export const decorateStepDataSourceRowForVisibilityAction = (args: {
  ...
  utilisationCommittedValues: Record<string, Record<string, FieldValue> | null | undefined>;
  buildStepDataSourceDraftKey: (config: any, parentRowId: string, sourceKey: string) => string;
  resolveLocalUtilisationQuantityForVisibility: (args: {
  ...
  const sourceKey = resolveUtilisationSourceItemKey(config, sourceRow);
  ...
  let localTotalUtilisedQuantity = 0;
  let committedTotalUtilisedQuantity = 0;
  ...
  parentRows.forEach(parentRow => {
    ...
    const committedValues = utilisationCommittedValues[draftKey] || null;
    const localQuantity = resolveLocalUtilisationQuantityForVisibility({
      draftValues,
    ...
    const committedQuantity = resolveUtilisationQuantityFromValues(
      committedValues || outputValues || null,
      selectedFieldId,
      quantityFieldId
    );

    localTotalUtilisedQuantity += localQuantity;
    committedTotalUtilisedQuantity += committedQuantity;
  });

  const visibilityScope = resolveSourceFirstAllocationUtilisationVisibilityScope({
    localTotalUtilisedQuantity,
    committedTotalUtilisedQuantity
  });

  return decorateSourceFirstAllocationRowForVisibility({
    row: sourceRow,
    availabilityConfig,
    localCurrentRecordUtilisedQuantity: visibilityScope.localCurrentRecordUtilisedQuantity,
    committedCurrentRecordUtilisedQuantity: visibilityScope.committedCurrentRecordUtilisedQuantity
  });
};
...
export const resolveStepDataSourceUtilisationStateForSourceAction = (args: {
  config: any;
  sourceKey: string;
  currentParentRowId?: string;
  ...
  if (!outputGroupId || !outputKeyFieldId || !quantityFieldId || !args.sourceKey) {
    return { totalUtilisedQuantity: 0, currentRowQuantity: 0 };
  }
  ...
  args.parentRows.forEach(parentRow => {
    ...
    const committedValues = args.utilisationCommittedValues[draftKey] || null;
    const quantity =
      args.mode === 'committed'
        ? args.resolveUtilisationQuantityFromValues(
            committedValues || outputValues || null,
            selectedFieldId,
            quantityFieldId
          )
        : args.resolveLocalUtilisationQuantityForVisibility({
            draftValues,
            outputValues: outputValues || null,
            committedValues,
            selectedFieldId,
  ...
  return { totalUtilisedQuantity, currentRowQuantity };
};
