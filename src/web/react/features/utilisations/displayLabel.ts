...
import { resolveUtilisationSourceItemKey, resolveUtilisationSourceKeyFieldId } from './sourceFields';

const normalizeIdValue = (raw: any): string => (raw === undefined || raw === null ? '' : String(raw).trim());

export const resolveUtilisationDisplayLabel = (
  config: any,
  ...
  const candidateFieldIds = [
    ...
    resolveUtilisationSourceKeyFieldId(config)
  ].filter(Boolean);
...
export const resolveUtilisationDisplayLabelFromCachedDataSources = (args: {
  ...
  for (const config of configs) {
    ...
    const match =
      items.find((item: Record<string, any>) => {
        ...
        if (resourceItemId && resolveUtilisationSourceItemKey(config, item) !== resourceItemId) return false;
        return true;
      }) || null;
    if (!match) continue;
    return resolveUtilisationDisplayLabel(config, match, resourceItemId || fallbackItemId);
  }
