...
const areLineItemRowsEqual = (a: LineItemRowState | undefined, b: LineItemRowState | undefined): boolean => {
  if (a === b) return true;
  if (!a || !b) return false;
  return (
    a.id === b.id &&
...
const areLineItemStatesEqual = (a: LineItemState, b: LineItemState): boolean => {
  if (a === b) return true;
  const aKeys = Object.keys(a || {}).sort();
  const bKeys = Object.keys(b || {}).sort();
  if (aKeys.length !== bKeys.length) return false;
  for (let keyIdx = 0; keyIdx < aKeys.length; keyIdx += 1) {
    const key = aKeys[keyIdx];
    if (key !== bKeys[keyIdx]) return false;
    const aRows = a[key] || [];
    const bRows = b[key] || [];
    if (aRows.length !== bRows.length) return false;
    for (let rowIdx = 0; rowIdx < aRows.length; rowIdx += 1) {
      if (!areLineItemRowsEqual(aRows[rowIdx], bRows[rowIdx])) return false;
    }
  }
  return true;
};
    ...
    language,
    {
      ...
      updateAutoLineItems: (
        ...
        setLineItems(prev => {
          ...
          const requestedParentGroupId = normalizeMetaString(opts?.lineItem?.groupId);
          const requestedParentRowId = normalizeMetaString(opts?.lineItem?.rowId);
          const resolveStableAutoEffectContextId = (reusableRow?: LineItemRowState): string => {
            const reusableValues = (reusableRow?.values || {}) as Record<string, unknown>;
            return resolveStableSelectionEffectContextId({
              nextContextId: meta.effectContextId,
              existingContextId: reusableRow?.effectContextId,
              effectId: normalizedEffectId,
              parentGroupId: reusableValues[ROW_PARENT_GROUP_ID_KEY] || reusableRow?.parentGroupId || requestedParentGroupId,
              parentRowId: reusableValues[ROW_PARENT_ROW_ID_KEY] || reusableRow?.parentId || requestedParentRowId
            });
          };
          ...
          const rebuiltAuto: LineItemRowState[] = presets.map(preset => {
            ...
            const values: Record<string, FieldValue> = mergeRebuiltAutoRowValues({
              existingValues: (reusableRow?.values || {}) as Record<string, FieldValue>,
              presetValues: preset as Record<string, FieldValue>
            });
            ...
            return {
              ...
              parentGroupId: opts?.lineItem?.groupId,
              autoGenerated: true,
              effectContextId: resolveStableAutoEffectContextId(reusableRow)
            };
          });
          ...
          const lineItemsChanged = !areLineItemStatesEqual(prev, recomputed);
          const valuesChanged = !areValueMapsEqual(latestValuesSnapshot, nextValues);
          if (!lineItemsChanged && !valuesChanged) return prev;
          latestLineItemsSnapshot = lineItemsChanged ? recomputed : prev;
          if (valuesChanged) {
            setValuesIfChanged(nextValues);
          }
          if (lineItemsChanged) {
          ...
          return lineItemsChanged ? recomputed : prev;
        });
      },
      deleteLineItemRows: (
        groupId: string,
      ...
      clearLineItems: (
        ...
        setLineItems(prev => {
          ...
          if (!removedIds.size) return prev;
