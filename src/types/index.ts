...
export interface FollowupCreateRecordEffect {
  /**
   * Optional stable identifier for referencing this effect from UI flows and templates.
   */
  id?: string;
  /**
  ...
  auditAction?: string;
  /**
   * Optional metadata describing how downstream records link back to the source record.
   *
   * This enables follow-up UIs (dialogs, summaries, reports) to resolve records that were
   * generated from the current source submission after the original save has already completed.
   */
...
export interface FollowupSubmitEffectSourceLink {
  /**
   * Target field storing the source record id.
   */
  sourceRecordIdFieldId: string;
  /**
   * Optional target field storing the source form/config key.
   */
...
export interface SubmissionAfterSubmitConfig {
  ...
  /**
   * Optional confirmation dialog shown before final submit starts.
   */
  confirmationDialog?: SystemActionGateDialogConfig;
  /**
   * Optional ordered conditional confirmation dialogs.
   *
   * The first matching case wins and falls back to `confirmationDialog` when none match.
   */
  ...
  /**
   * Optional single-action dialog shown after successful pre-actions when matching submit effects
   * created downstream records that the user should review before navigation.
   */
  generatedRecordsDialog?: StepGeneratedRecordsDialogConfig;
}
...
export interface DataSourceConfig {
  ...
  mapping?: Record<string, string>; // optional map from source column -> target field id / special keys like value or label
...
export interface StepsConfig {
   ...
   * The final step prefers the form's submitButtonLabel and falls back to this value when needed.
   */
...
export interface StepMilestoneActionConfig {
  ...
  confirmationDialog?: SystemActionGateDialogConfig;
  /**
   * Optional conditional confirmation dialogs evaluated in order against the current form state.
   * The first matching case wins and falls back to `confirmationDialog` when none match.
   */
  confirmationDialogCases?: StepMilestoneDialogCaseConfig[];
  /**
  ...
  feedbackDialog?: SystemActionGateDialogConfig;
  /**
   * Optional post-success dialog showing records generated by the milestone's follow-up actions.
   *
   * Intended for end-of-flow guidance such as "label the leftovers you just created" while
   * keeping the dialog content fully driven by config.
   */
  generatedRecordsDialog?: StepGeneratedRecordsDialogConfig;
  /**
   * Optional navigation target applied after the milestone succeeds (and after any generated-records
   * dialog is dismissed).
   *
   * - current: keep the current view/step
   * - form: open the form view
   * - summary: open the summary view
   * - list: return to the home/list view
   */
  navigateToAfterSuccess?: 'current' | 'form' | 'summary' | 'list';
  /**
...
export interface StepMilestoneDialogCaseConfig {
  when?: WhenClause;
  dialog: SystemActionGateDialogConfig;
}

export interface StepGeneratedRecordsDialogConfig {
  /**
   * Optional effect ids to include. When omitted, all generated records from the milestone batch are eligible.
   */
  submitEffectIds?: string[];
  /**
   * Optional target form key filter. Useful when the milestone generates records in multiple forms.
   */
  targetFormKey?: string;
  /**
   * Dialog title shown above the generated record list.
   */
  title?: LocalizedString;
  /**
   * Intro text shown before the generated record lines.
   */
  message?: LocalizedString;
  /**
  ...
  /**
   * Optional label override for the acknowledgement button.
   */
  confirmLabel?: LocalizedString;
}
...
export interface CopyCurrentRecordLineItemProfile {
   ...
   * Supports `$row.FIELD_ID`, `$parent.FIELD_ID`, and `$top.FIELD_ID` references.
   */
...
export interface FollowupActionResult {
  ...
  submitEffects?: {
    ...
    updated?: number;
    operation?: 'create' | 'update' | string;
    generatedRecords?: SubmitEffectGeneratedRecord[];
  };
}

export interface SubmitEffectGeneratedRecord {
  effectId?: string;
  targetFormKey: string;
  recordId: string;
  values?: Record<string, any>;
